...
function myWidget(viewer,scene,ellipsoid)
{
	...
	{   var divele=document.createElement('div');
		divele.innerHTML='<span id=\"long\">Longitude：</span><span id=\"lat\"  style=\"padding-left: 10px;\">    \tLatitude：</span><span id=\"height\"  style=\"padding-left: 10px;\">    \tAltitude：</span><span id=\"viewheight\"  style=\"padding-left: 10px;\">    \tView Elevation：</span>';
/*
		document.write("<div class=\"cesium-toolbar2\" id=\"cameraView\"><span id=\"long\">Longitude：</span><span id=\"lat\"  style=\"padding-left: 10px;\">    \tLatitude：</span><span id=\"height\"  style=\"padding-left: 10px;\">    \tAltitude：</span><span id=\"viewheight\"  style=\"padding-left: 10px;\">    \tView Elevation：</span></div>\"");
*/
		divele.setAttribute('id','cameraView');
		divele.setAttribute('class','cesium-toolbar2');
		document.getElementById('cesiumContainer').appendChild(divele);
			...
			if (cartesian||Cesium.defined(position)) {
				...
				if(this.viewer.camera._mode === Cesium.SceneMode.SCENE3D&&Cesium.defined(position)){
					...
					promise.then(function() {
						...
						if(height)
							heightString = height;
						else heightString =  "Undefined";
						document.getElementById("height").innerHTML='\t'+"Altitude: "+heightString+' Meter';
					  }).otherwise(function(error){
						  //Display any errrors encountered while loading.
						  document.getElementById("height").innerHTML="	\tAltitude: Undefined";
					  });
				}
				else document.getElementById("height").innerHTML="";
				document.getElementById("long").innerHTML="Longtitude: "+longitudeString;
				document.getElementById("lat").innerHTML="    \tLatitude: "+latitudeString;
				if(cameraheight0>1000)
					document.getElementById("viewheight").innerHTML= "    \tView Elevation: "+(cameraheight0/1000).toFixed(2)+" km";
				else document.getElementById("viewheight").innerHTML= "    \tView Elevation: "+cameraheight0.toFixed(0)+" Meter";
				//document.getElementById("viewheight").innerHTML="    \tView Elevation: "+cameraheight+' Meter';
				
			} else {
				document.getElementById("long").innerHTML="Longtitude：";
				document.getElementById("lat").innerHTML="    \tLatitude：";
				document.getElementById("height").innerHTML="    \tAltitude：";
...
//当前相机信息获取
	...
	function showScalebar(){
		...
		scaleContainer.setAttribute('style','width:'+120+'px;height:'+30+'px;');
		...
		scaletext.setAttribute('style','width:'+120+'px;height:'+30+'px;background:rgba(255,255,255,0);');
		cesiumContainer.appendChild(scaletext);
		document.getElementById("scaletext").innerHTML='<div id=\"scaletext\" style=\"display: inline-block;\">Scale</div>';
			...
			if(viewchanged){
				if(this.viewer.sceneMode == Cesium.SceneMode.SCENE3D||this.viewer.sceneMode == Cesium.SceneMode.COLUMBUS_VIEW){
					...
					if(position1&&position2){
						...
						if(distance>3000)
						document.getElementById("scaletext").innerHTML= (distance/1000).toFixed(0)+" km";
						else document.getElementById("scaletext").innerHTML= distance.toFixed(0)+" Meter";
					}
					else document.getElementById("scaletext").innerHTML="Scale";
				}
				else{
...
/* 			handler.setInputAction(function(movement) {
	...
	function doNavi()
	{
		...
		navidiv.setAttribute('class','navigation-controls');
		navidiv.innerHTML='<div title="Zoom In" class="navigation-control" onclick="clickZoomIn()">\
		...
		document.getElementById('cesiumContainer').appendChild(navidiv);
...
var Graticule = (function() {
	...
	var specLines = {'Arctic Circle' : 66.5,
